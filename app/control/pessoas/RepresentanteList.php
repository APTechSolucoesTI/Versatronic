...
class RepresentanteList extends TPage
{
    private $form; // form
    ...
    private static $formName = 'form_RepresentanteList';
    ...
    public function __construct($param = null)
    {
        ...
        $this->form->setFormTitle("Listagem de representantes");
        $this->limit = 20;
        ...
        $ativo_col = new TCombo('ativo_col');
        ...
        $ativo_col->setChangeAction(new TAction([$this, 'onSearch'], ['static'=>'1']));

        $ativo_col->addItems(["S"=>" Sim","N"=>" Não"]);
        $ativo_col->enableSearch();
        ...
        $row1 = $this->form->addFields([new TLabel("Id:", null, '14px', null, '100%'),$id],[new TLabel("Codigo:", null, '14px', null, '100%'),$codigo],[new TLabel("Usuario do Sistema", null, '14px', null, '100%'),$system_user_id],[new TLabel("Ativo:", null, '14px', null, '100%'),$ativo]);
        $row1->layout = [' col-sm-3',' col-sm-3',' col-sm-3',' col-sm-3'];

        $row2 = $this->form->addFields([new TLabel("Razão Social:", null, '14px', null, '100%'),$razao_social],[new TLabel("Email:", null, '14px', null, '100%'),$email],[new TLabel("Telefone:", null, '14px', null, '100%'),$telefone]);
        ...
        if(TSession::getValue('RepresentanteList_expand_start_hidden') === false)
        {
            $startHidden = false;
        }
        elseif(TSession::getValue('RepresentanteList_expand_start_hidden') === true)
        {
            $startHidden = true; 
        }
        ...
        $column_cpf_cnpj_transformed = new TDataGridColumn('cpf_cnpj', "CNPJ", 'left');
        ...
        $action_onEdit = new TDataGridAction(array('RepresentanteForm', 'onEdit'));
        $action_onEdit->setUseButton(false);
        $action_onEdit->setButtonClass('btn btn-default btn-sm');
        $action_onEdit->setLabel("");
        $action_onEdit->setImage('fas:user #333333');
        ...
        $td_ativo_col = TElement::tag('td', $ativo_col);
        $tr->add($td_ativo_col);
        ...
        $this->datagrid_form->addField($ativo_col);
        ...
        $button_atualizar->setAction(new TAction(['RepresentanteList', 'onRefresh']), "Atualizar");
        ...
        $button_limpar_filtros->setAction(new TAction(['RepresentanteList', 'onClearFilters']), "Limpar filtros");
        ...
        $button_cadastrar->setAction(new TAction(['RepresentanteForm', 'onShow']), "Cadastrar");
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['RepresentanteList', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-csv #00b894' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['RepresentanteList', 'onExportXls'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['RepresentanteList', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XML", new TAction(['RepresentanteList', 'onExportXml'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-code #95a5a6' );
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Pessoas","Representantes"]));
        }
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->ativo_col) AND ( (is_scalar($data->ativo_col) AND $data->ativo_col !== '') OR (is_array($data->ativo_col) AND (!empty($data->ativo_col)) )) )
        {

            $filters[] = new TFilter('ativo', '=', $data->ativo_col);// create the filter 
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for Representante
    ...
    public static function onExpandForm($param = null)
    {
        try
        {
            ...
            if(TSession::getValue('RepresentanteList_expand_start_hidden') === false)
            {
                TSession::setValue('RepresentanteList_expand_start_hidden', true);
            }
            elseif(TSession::getValue('RepresentanteList_expand_start_hidden') === true)
            {
                TSession::setValue('RepresentanteList_expand_start_hidden', false);
            }
            else
            {
                TSession::setValue('RepresentanteList_expand_start_hidden', !$startHidden);
            }
