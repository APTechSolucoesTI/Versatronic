...
class LogCrontabHeaderList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $system_unit_id = new TDBCombo('system_unit_id', 'minicrm', 'SystemUnit', 'id', '{name}','name asc' , $criteria_system_unit_id );
        ...
        $column_data_hora_transformed = new TDataGridColumn('data_hora', "Data e Hora", 'left' , '10%');
        $column_system_unit_name = new TDataGridColumn('system_unit->name', "Unidade", 'left' , '10%');
        ...
        {
            if ($object->status === 0 || strpos($object->mensagem, 'Exception') === false || TSession::getValue('userid') == 1) {
                return $value;
            } else {
        ...
        $column_status_transformed->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
            switch($value){
                ...
                default: return '<span class="label label-warning" style="width:auto !important;">Sem Status</span>'; break;
            }
        ...
        $action_onDelete = new TDataGridAction(array('LogCrontabHeaderList', 'onDelete'));
        $action_onDelete->setUseButton(false);
        $action_onDelete->setButtonClass('btn btn-default btn-sm');
        $action_onDelete->setLabel("Excluir");
        $action_onDelete->setImage('fas:trash-alt #F44336');
        $action_onDelete->setField(self::$primaryKey);
        $action_onDelete->setDisplayCondition('LogCrontabHeaderList::canDelete');
        $action_onDelete->setParameter('key', '{id}');
        ...
        $button_teste = new TButton('button_button_teste');
        $button_teste->setAction(new TAction(['LogCrontabHeaderList', 'onTeste']), "Teste");
        $button_teste->addStyleClass('btn-default');
        $button_teste->setImage('far:circle #000000');

        $this->datagrid_form->addField($button_teste);
        ...
        $head_left_actions->add($button_teste);
    ...
    public function onDelete($param = null) 
    {
        if(isset($param['delete']) && $param['delete'] == 1)
        {
            try
            {
                ...
                TToast::show('info', 'Registro excluído', 'topRight');
                TApplication::loadPage('LogCrontabHeaderList', 'onShow');
            }
    ...
    public function onDiaria($param = null) 
    {
        try 
        {
            AtualizacaoService::atualizarTudo();
            $this->onReload();
    ...
    public function onTeste($param = null) 
    {
        try 
        {
            if(TSession::getValue('userid') == 1){
                $numero = '019917';
                SigissWeb::inserirMovimentacaoXml($numero,Coligada::VERSATRONIC);
                SigissWeb::cancelarNf($numero, Coligada::VERSATRONIC, 'Teste');
                SigissWeb::obtemXmlRps($numero,Coligada::VERSATRONIC);
                SigissWeb::obtemXmlNf($numero,Coligada::VERSATRONIC);
                SigissWeb::obterPdf($numero,Coligada::VERSATRONIC);
                SigissWeb::enviarEmail($numero, Coligada::VERSATRONIC, 'N');
            }
            $this->onRefresh();
            //</autoCode>
        }
